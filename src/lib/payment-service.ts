...
export class PaymentService {
  ...
  static async createPayPalOrder(
    ...
    try {
      const request = new paypal.orders.CreateOrderRequest();
  ...
  static async capturePayPalOrder(order_id: string): Promise<PaymentResult> {
    try {
      const request = new paypal.orders.CaptureOrderRequest(order_id);
      request.requestBody({});
  ...
  static async completeTicketPurchase(paymentId: string): Promise<void> {
    try {
      ...
      try {
        ...
        const ticket = tickets.find((t: any) => t.payment_id === paymentId);
